        ...
        while not valid_square:
            ...
            try:
                ...
                if val not in game.available_moves():
                    raise ValueError
                valid_square = True #if these are successfull, then yey!
            except ValueError:
...
class geniusplayer(Player):
    ...
    def get_move(self, game):
        if len(game.available_moves())== 9:
            square = random.choice(game.available_moves())#randomly choose a spot bc all spots are open
        else: #get move from algo
           square = self.minimax(game, self.letter)['position']
        return square 

    def minimax(self, state, player):
        max_player = self.letter #yourself
        other_player = "O" if player == 'X' else 'X' #identifying letters to players
        ...
        for possible_move in state.available_moves():
            #step1 make a move from available moves
